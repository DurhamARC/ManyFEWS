...
for i in range(intervalNum):
    ...
    aggregatedData[i, 5] = np.mean(intervalData[:, 1])  # mean result of precipitation
    aggregatedData[i, 1] = np.max(intervalData[:, 2])  # find the maximum temperature
    aggregatedData[i, 2] = np.min(intervalData[:, 2])  # find the minimum temperature
